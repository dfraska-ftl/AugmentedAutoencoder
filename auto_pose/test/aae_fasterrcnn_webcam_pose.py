...
from auto_pose.test.webcam_video_stream import WebcamVideoStream
from auto_pose.ae.utils import get_dataset_path
from auto_pose.test.aae_fasterrcnn_pose_estimator import (
    AeFasterRCNNPoseEstimator, PoseEstimatorArgs)

class ResultsRenderer:
    def __init__(self, ae_pose_est: AeFasterRCNNPoseEstimator, workspace_path):
        self._ae_pose_est = ae_pose_est
        
        # Create a color LUT for the boxes
        self._color_dict = [
            (0, 255, 0), (0, 0, 255), (255, 0, 0), (255, 255, 0)] * 10
        
        ply_model_paths = [str(train_args.get('Paths', 'MODEL_PATH'))
                           for train_args in ae_pose_est.all_train_args]
        self._cad_reconst = [str(train_args.get('Dataset', 'MODEL'))
                       for train_args in ae_pose_est.all_train_args]
        
        self._renderer = meshrenderer.Renderer(
            ply_model_paths, samples=1, 
            vertex_tmp_store_folder=get_dataset_path(workspace_path),
            vertex_scale=float(1)) # float(1) for some models
        
    def render(self, image, all_pose_estimates, all_class_idcs, boxes, scores, labels):
        ae_pose_est = self._ae_pose_est
        
        bgr, _depth, _bbs = self._renderer.render_many(
        ...
        for label,box,score in zip(labels, boxes, scores):
            ...
            print(label)
            cv2.putText(
                image_show, '%s: %1.3f' % (label, score), (xmin, ymax + 20),
                cv2.FONT_ITALIC, .5, self._color_dict[int(label)], 2)
            cv2.rectangle(
                image_show, (xmin, ymin), (xmax, ymax),
                (255, 0, 0), 2)
...
def main():
    ...
    parser.add_argument("-test_config", type=str, required=False, default='ae_webcam_fasterrcnn.cfg')
    ...
    ae_pose_est = AeFasterRCNNPoseEstimator(
        PoseEstimatorArgs(test_configpath), workspace_path)
    
    videoStream = WebcamVideoStream(0, ae_pose_est._width, ae_pose_est._height).start()
    
    if args.vis:
        resultsRenderer = ResultsRenderer(ae_pose_est, workspace_path)
    ...
    while videoStream.isActive():
        ...
        if args.vis:
            image_show = resultsRenderer.render(
                image, all_pose_estimates, all_class_idcs, boxes, scores, labels)
